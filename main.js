...
rgbButton.addEventListener("click", () => {
  ...
  if (rgb) {
    clearInterval(rgb);
    setTimeout(() => {
      rootCSS.style.setProperty("--main-color", "#009879");
    }, 0);
    rgb = false;
  } else {
...
const characterNameUnderline = document.querySelector(
  ".character-name-underline"
);
const prevBtn = document.querySelector(".prev-btn");

const showCharacterImg = (arr) => {
  ...
  characterImg.forEach((image) => {
    image.addEventListener("click", () => {
      ...
      nextBtn.addEventListener("click", () => {
        ...
        fullImgSkin.setAttribute(
          "src",
          `full-img/skins/${imgSrc}s01_idle_normal.png`
        );
...
fullImgModal.addEventListener("click", (e) => {
  if (
    e.target.classList.contains("full-img-modal") ||
...
const filterCharacters = () => {
  const filteredCharacters = epic7Characters
    .filter((letterCharacter) => {
...
const sortCharacters = (prop, order) => {
  if (order) {
...
tableColumns.forEach((column) => {
  column.addEventListener("click", (e) => {
    ...
    const sortOrder = column.classList.contains("alphabeticalOrder")
      ? true
      : false;
    sortCharacters(sortName, sortOrder);
    tableColumns.forEach((column) => {
      column.classList[e.target == column ? "toggle" : "remove"](
        "alphabeticalOrder"
      );
    });
  });
